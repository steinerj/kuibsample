...
export class RestDataService<T> extends DataService<T> {
    ...
    protected readRequest(state: State): Observable<any> {
        ...
        return this.request(this.config.actions.read.method, url, { observe: 'response' });
    }
    ...
    protected createRequest(data: any): Observable<any> {
        ...
        return this.request(this.config.actions.create.method, url, {
            body: data,
            observe: 'response'
        });
    }
    ...
    protected updateRequest(data: any): Observable<any> {
        ...
        return this.request(this.config.actions.update.method, url, {
            body: data,
            observe: 'response'
        });
    }
    ...
    protected removeRequest(data: any): Observable<any> {
        ...
        return this.request(this.config.actions.remove.method, url, { observe: 'response' });
    }

    protected parseResponse(response: HttpResponse<Object>): ModelDataResult<T> {
        const result = new ModelDataResult<T>();
        result.data = this.mapData(this.config.dataProperty ? response.body[this.config.dataProperty] : response.body);
